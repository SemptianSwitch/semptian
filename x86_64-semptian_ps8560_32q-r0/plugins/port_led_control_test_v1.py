...
def port_led_set(pst,val):
    ret = False
    if os.system("echo "+str(val)+" > /sys/bus/i2c/devices/4-0011/hwmon/hwmon1/port"+str(pst)+"_led") == 0:
        if os.popen("cat /sys/bus/i2c/devices/4-0011/hwmon/hwmon1/port"+str(pst)+"_led").read().split("\n")[0] == str(val):
            ret = True
    return ret
...
def led_polling(address):
    if os.system("sudo chmod 777 /sys/bus/i2c/devices/4-0011/hwmon/hwmon1/port*_led") != 0:
